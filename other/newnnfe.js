...
http.get(url, function (res) {
    ...
    res.on('end', function () {
        ...
        for (var i in data.data) {
            var inputvec = [];
            var outputvec = [];
            //console.log(data.data[i].platform);
            if (data.data[i].platform == "desktop") {
                inputvec.push(1, 0, 0);
            }
            else if (data.data[i].platform == "app") {
                inputvec.push(0, 1, 0);
            }
            else {
                inputvec.push(0, 0, 1);
            ...
            outputvec.push(1 / (1 + Math.exp(-(data.data[i].spend-70000) / 50000)));
            console.log(1 / (1 + Math.exp(-(data.data[i].spend-70000) / 50000)));
        ...
        //    console.error(err)
        //})
        console.log(myNet.activate.toString());
    });
}).on('error', function (e) {
